...
app.post("/api/v1/login", (req, res) => {
  ...
  if (accessCode !== accessCodeGenerated || !branch) {
...
app.get("/api/v1/orders-summary", validateToken, (req, res) => {
  const { dateFrom, dateTo, emergency } = req.query;
  console.log({ dateFrom, dateTo, emergency });
  ...
  if (!emergencyOrderData) {
    return res.status(404).json({
      message: "Emergencies orders not found",
    });
  }
...
app.post("/api/v1/create-order", async (req, res) => {
  ...
  try {
    const response = await sendRabbitMQMessage(
      "API for Orion",
      "A new emergency order was created"
    );
